            ...
            <div class="row">
                ...
                <form class="form-inline" name="frmSearch" method="post" action="<?php echo $_SERVER['SCRIPT_NAME'];?>">
                    ...
                    <select class="custom-select d-block " id="TypeKeyword" name="TypeKeyword">
                        ...
                        AND type.ID LIKE '%".$TypKeyword."%' ORDER BY item.IDs ASC "  ;
...
<div style="display: flex;justify-content: center;">
    <table class="table center tcenter newfont" style="width: 1400px">
        <tr style="background-color: #ff9999">
            ...
            <th scope="col" class="zen" >ชื่อครุภัณฑ์</th>
        ...
        <table class="table table-striped">
            <tbody>
                ...
                <tr>
                    ...
                    <td scope="row" class="zen" ><?=$row['ItemName']?></td>  
                    <td scope="row" class="zen" style="width: 140px;"><?=$row['Detail']?></td>  
                    <td scope="row" class="zen" style="width: 140px;"><?=$row['TypeName']?></td>  
                    <td scope="row" class="zen tcenter" style="width: 60px;"><?=$row['StatusName']?></td>  
                    <td scope="row" class="zen tcenter" style="width: 80px;"><?=$row['Building']?></td>  
                    <td scope="row" class="zen" style="width: 100px;"><?=$row['Price']?></td>  
                    <td scope="row" class="zen" style="width: 70px;"><?=$row['TeacherRight']?></td>  
                    <td scope="row" class="zen" style="width: 70px;"><?=$row['StaffRight']?></td>  
                    <td scope="row" class="zen" style="width: 70px;"><?=$row['StudentRight']?></td>  
                    <td scope="row" class="zen" style="width: 120px;"><?=$row['Add_Date']?></td>  
                    <td scope="row" class="zen" style="width: 120px;"><?=$row['Edit_Date']?></td>  
                    <td scope="row" class="zen" style="width: 115px;">
                            <?php if($row['StatusName']=="ถูกยืม"){ ?>
                                <button type="button" class="btn btn-warning" style="width: 70px" disabled>ถูกยืม</button>
                            <?php }elseif($row['StatusName']=="ปกติ"){ ?>
                                <a href='editform.php?ItemID=<?php echo $row['ItemID'] ?>&&IDs=<?php echo $row['IDs'] ?> '><button
                                    type="button" class="btn btn-info" style="width: 70px">แก้ไข</button></a> 
                                <a href='FormCheckMail.php?IDs=<?php echo $row['IDs'] ?> '><button
                                type="button" class="btn btn-secondary" style="width: 70px">ยืม</button></a> 
                            ...
                            onclick="return confirm('คุณต้องการลบครุภัณฑ์นี้ใช่หรือไม่!!!')"><button name="delete"
                                type="submit" class="btn btn-danger" style="width: 70px">ลบ</button></a>  
 
                            <?php }else{ ?>
                                <a href='editform.php?ItemID=<?php echo $row['ItemID'] ?>&&IDs=<?php echo $row['IDs'] ?> '><button
                                    type="button" class="btn btn-info" style="width: 70px">แก้ไข</button></a> 
                                <a href='FormCheckMail.php?IDs=<?php echo $row['IDs'] ?> '><button
                                type="button" class="alert alert-dark" style="width: 70px" disabled>ไม่พร้อม</button></a> 
                                <a href='FuncDelete.php?ItemID=<?php echo $row['ItemID'] ?> '
                            onclick="return confirm('คุณต้องการลบครุภัณฑ์นี้ใช่หรือไม่!!!')"><button name="delete"
                                type="submit" class="btn btn-danger" style="width: 70px">ลบ</button></a>  
